...
//initializing objects of LexicalAnalyzer, Token and TokenType

LexicalAnalyzer lexer;
Token token;
TokenType tempTokenType;

//variables to check type
int eType;
int eCount = 4;

//scope resolve struct
struct scopeResolve {
  char * scope;
  scopeResolve * next;
};
...
//adding sTableEntry to list
void addList(std::string name, int line, int type) {
  if (symbolTable == NULL) { 
    // dynamically initialzing table and entry objects
  ...
  } else { //adding to already present table
...
int Search_List(std::string n) {
  ...
  bool found = false;
  if (symbolTable == NULL) {
    if (ifToken) { //searching for a token that isnt present will add it to the list
      addList(n, token.line_no, 3); 
      eCount++;
      return (3);
    }
    addList(n, token.line_no, eCount);
    eCount++;
    return (4);
  } else {
    while (temp -> next != NULL) {
...
//printing token type and line number
void Token::Print() {
  cout << "{" << this -> lexeme << " , " <<
...
//previous assignment code for lexical analyzer and parsing functions with edits 
//=============================================================================
...
int parse_expression(void) {
  ...
  if (token.token_type == ID || token.token_type == NUM || token.token_type == REALNUM || token.token_type == TR || token.token_type == FA) {
    ...
    tempI = parse_primary();
    //cout<<token.line_no<<"  ::  "<<token.lexeme<<"  :::  "<<tempI<<endl;
    ...
    //cout<<"leftE : "<<tempI1<<"  ::  rightE : "<<tempI2<<" :: tempI : "<<tempI<<" :: token : "<<token.line_no<<" :: "<<token.lexeme<<endl; 
    if ((tempI1 != tempI2) || isExpress(tempI)) {

      if (tempI == 15 || tempI == 16 || tempI == 17 || tempI == 18) {
        if (tempI1 <= 3 && tempI2 > 3) {
    ...
    if (tempI == 19 || tempI == 20 || tempI == 21 || tempI == 22 || tempI == 23 || tempI == 26) {
      tempI = 3;
    } else {
      tempI = tempI2;
    }
...
int parse_assstmt(void) {
  ...
  token = lexer.GetToken();
  if (token.token_type == ID) {
    lType1 = Search_List(token.lexeme);
    //cout<<"lType1  : "<<lType1<<"  :: token :"<<token.lexeme<<endl;
    token = lexer.GetToken();
